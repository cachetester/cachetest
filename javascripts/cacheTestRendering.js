...
function renderRequestResponseRow(requestResponseDefinitionObject, testCaseName){
    ...
    for(var headerFieldName in responseHeaderFields){
        if(headerFieldName == "X-Id" || headerFieldName == "x-id" || headerFieldName == "Id" || headerFieldName == "id"){
            responseId = responseHeaderFields[headerFieldName];
    ...
    for(var headerFieldName in requestHeaderFields){
        if(headerFieldName == "X-Id"){
            ...
            requestHeaderFieldsString += `<span class="${responseHeaderFields[headerFieldName]}"><strong>${headerFieldName}</strong>` + ": "+requestHeaderFields[headerFieldName] + "</span><br/>";
        } else {
    ...
    row +=       `<td class="${requestId} message request" rid="${requestId}"><span class="startLine" rid="${requestId}"><span class="method">${method}</span> <span class="url">${url}</span> <span class="protocol">${protocolRequest}/${versionRequest}</span></span> <i class="glyphicon glyphicon-collapse-down toggleCollapse"></i> ${requestHeaderFieldsString}</td>`;
    row +=       `<td class="${responseId} message response" rid="${responseId}"><span class="startLine" rid="${responseId}"><span class="statusCode">${statusCode} ${reasonPhrase}</span> <span class="protocol">${protocolResponse}/${versionResponse}</span></span> <i class="glyphicon glyphicon-collapse-down toggleCollapse"></i> ${responseHeaderFieldsString}</td>`;
    if(cacheHitHeader || cacheHitBody){
        row +=       `<td><span class="glyphicon glyphicon-ok" aria-hidden="true" style="color:green"></span> <strong>${cacheHitType}</strong></td>`;
    } else {
        row +=       `<td><span class="glyphicon glyphicon-remove" aria-hidden="true" style="color:red"></span></td>`;
    }

    if(compliance){
        row +=       `<td><span class="glyphicon glyphicon-ok" aria-hidden="true" style="color:green"></span></td>`;
        ...
        for (var i = 0 ; i < complianceErrors.length ; i++) {
            errorDesc += `<span class="label label-info">${complianceErrors[i]}</span><br/>`;
        }
        row += `<td><span class="glyphicon glyphicon-remove" aria-hidden="true" style="color:red"></span><br/>${errorDesc}</td>`;
    ...
    row +=     "</tr>";
    row +=`<tr class="info"><td colspan="5">${pause} second(s) later</td></tr>`;
    return row;
}
...
$("body").on("click",".toggleCollapse",function(){
    ...
    //$("#testCasesTable").find("td."+rid).toggleClass("danger");
    var td = $(this).parent();
    var tr = td.parent();
...
$("body").on("click",".startLine",function(){
    ...
    messages.each(function(index,element){
        ...
        var headerHasSameId = requestHeaderIdOfElement == requestHeaderId;
        var bodyHasSameId = requestHeaderIdOfElement == responseBodyId;
